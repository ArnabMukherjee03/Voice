...
export default function LoginPage(){
    ...
    const Login = async()=>{
        try {
            ...
            setMe(response.data.currentUser)
            setAuth(true);
        ...
        <div className="">
             <div className="flex flex-row-reverse min-h-screen ">
                    ...
                    <div className="w-1/2 flex flex-col items-center mt-[70px]">
                        ...
                        <Image src={Voicelogo} className="w-14" alt="Voice"/>
                            <h1 className="text-2xl">Welcome Back to Voice!</h1>
                            <p className="text-sm text-gray-500 tracking-wide">Let Your Voice Be Heard: Log In to Your Account</p>
                        </div>
                        {/* Form */}
                        <div className="flex flex-col w-1/2 gap-2 mt-6">
                            <div className="">
                            <label htmlFor="email">Email</label>
                            <input className="h-[40px] rounded-md outline-none w-full border-[1px] border-gray-300 mt-1 ps-4" autoComplete="off" placeholder="you@awesome.com" type="email" name="email" id="email" onChange={(e)=>{setData({...data,email: e.target.value})}} />
                            </div>
                            <div className="">
                            <label htmlFor="password">Password</label>
                            <input className="h-[40px] rounded-md outline-none w-full border-[1px] border-gray-300 mt-1 ps-4" autoComplete="off" placeholder="Password" type="password" name="password" id="password" onChange={(e)=>{setData({...data,password: e.target.value})}} />
                            </div>
                            <div className={`w-full bg-black rounded-md text-white text-center py-2 mt-2  ${loader ? "cursor-not-allowed opacity-80":"cursor-pointer"}`} onClick={Login}>
                            {loader ? 
                                "loading..." : "Log in"}
                            </div>
                        </div>
                        ...
                        <div className="w-1/2">
                            ...
                            <div className="text-center mt-2 flex justify-center gap-1">Don&apos;t have an account? <p className="relative overflow-hidden Underline cursor-pointer"><Link href="/signup">Sign up for free</Link></p></div>
                        </div>
                    </div>
                    {/* Testimonial */}
                    <div className="w-1/2 flex items-center  p-8">
                            ...
                            autoplay={{
                              ...
                              >
                                ...
                                <div className="flex justify-start mr-4 gap-2">
